...
Zbiera naglowki + tresci z 4 kategorii
...
if TEST_MODE:
    ...
    CATEGORIES_TO_SCRAPE = ['pralka', 'zmywarka', 'suszarka', 'piekarnik']
    ...
    if checkpoint_file.exists():
        try:
            with open(checkpoint_file, 'r', encoding='utf-8') as f:
                ...
                logger.info(f"Zaladowano {len(urls)} juz zebranych URL-i")
                return urls
...
def fetch_thread_content(url: str):
    try:
...
def scrape_category(category_name: str, search_term: str):
    logger.info(f"\n{'='*80}")
